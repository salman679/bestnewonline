...
import { FaTimes, FaTruck, FaBox, FaUser, FaCreditCard, FaMapMarkerAlt, FaCopy, FaCheckCircle } from 'react-icons/fa';
...
const OrderDetailsModal = ({ order, onClose, onStatusUpdate, statusOptions, handleCopyOrderId, className }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#00000062] flex items-center justify-center z-50 overflow-hidden">
      <div className={`bg-white dark:bg-gray-800 rounded-lg w-full h-[90vh] max-w-4xl mx-auto my-2 sm:my-8 shadow-xl ${className} flex flex-col`}>
              ...
              <div className="w-full sm:w-auto">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <button
                    ...
                    title="Copy Order ID"
                  >
                    {copiedField === 'Order ID' ? <FaCheckCircle className="w-4 h-4 text-green-500" /> : <FaCopy className="w-4 h-4" />}
                  </button>
                </div>
              ...
              <div className="w-full p-4 sm:p-8 bg-white rounded-xl flex-col justify-start items-start gap-3 sm:gap-5 flex">
                ...
                <div className="w-full flex-col justify-center items-center">
                  <ol className="flex flex-col sm:flex-row items-start justify-between w-full gap-4 sm:gap-1">
                    <li className={`group flex relative justify-start  ${order.status === 'pending' || order.status === 'processing' || order.status === 'shipped' || order.status === 'completed' ? 'after:bg-green-600' : 'after:bg-gray-500'} after:inline-block after:absolute md:after:top-7 after:top-3 xl:after:left-44 lg:after:left-40 md:after:left-36`}>
                      <div className="w-full mr-1 block z-10 flex flex-col items-center justify-start gap-1">
                        <div className="justify-center items-center gap-1.5 inline-flex">
                          <h5 className={`text-center ${order.status === 'pending' || order.status === 'processing' || order.status === 'shipped' || order.status === 'completed' ? 'text-green-600' : 'text-gray-500'} text-lg font-medium leading-normal`}>
                            Order Placed
                          </h5>
                          {(order.status === 'pending' || order.status === 'processing' || order.status === 'shipped' || order.status === 'completed') && (
                    ...
                    <li className={`group flex relative justify-start  ${order.status === 'processing' || order.status === 'shipped' || order.status === 'completed' ? 'after:bg-green-600' : 'after:bg-gray-500'} after:inline-block after:absolute md:after:top-7 after:top-3 xl:after:left-44 lg:after:left-40 md:after:left-32`}>
                      <div className="w-full mr-1 block z-10 flex flex-col items-center justify-start gap-1">
                        <div className="justify-center items-center gap-1.5 inline-flex">
                          <h5 className={`text-center ${order.status === 'processing' || order.status === 'shipped' || order.status === 'completed' ? 'text-green-600' : 'text-gray-500'} text-lg font-medium leading-normal`}>
                           Order Processing
                          </h5>
                        ...
                        <h6 className="text-center text-gray-500 text-base font-normal leading-relaxed">
                          {order.status === 'processing'  ? formatDate(order.updatedAt) : '-'}
                        </h6>
                      </div>
                    </li>

                    <li className={`group flex relative justify-start  ${order.status === 'shipped' || order.status === 'completed' ? 'after:bg-green-600' : 'after:bg-gray-500'} after:inline-block after:absolute md:after:top-7 after:top-3 xl:after:left-44 lg:after:left-40 md:after:left-32`}>
                      <div className="w-full mr-1 block z-10 flex flex-col items-center justify-start gap-1">
                        <div className="justify-center items-center gap-1.5 inline-flex">
                          <h5 className={`text-center ${order.status === 'shipped' || order.status === 'completed' ? 'text-green-600' : 'text-gray-500'} text-lg font-medium leading-normal`}>
                           Order Shipped
                          </h5>
                        ...
                        <h6 className="text-center text-gray-500 text-base font-normal leading-relaxed">
                          {order.status === 'shipped' ? formatDate(order.updatedAt) : '-'}
                        </h6>
                      </div>
                    </li>
                    ...
                    <li className="group flex relative justify-start">
                      <div className="w-full block z-10 flex flex-col items-center justify-start gap-1">
                        <div className="justify-center items-center gap-1.5 inline-flex">
                          <h5 className={`text-center ${order.status === 'completed' ? 'text-green-600' : 'text-gray-500'} text-lg font-medium leading-normal`}>
                            Order Delivered
                          </h5>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        title="Copy User ID"
                      >
                        {copiedField === 'User ID' ? <FaCheckCircle className="w-4 h-4 text-green-500" /> : <FaCopy className="w-4 h-4" />}
                      </button>
                    </div>
                    <p className="text-sm text-gray-900 dark:text-white">{order?.userId || 'N/A'}</p>
                  </div>
                  {order?.notes && (
                    <div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-3 sm:p-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-900 dark:text-white">{order?.address || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">District</p>
                    <p className="text-sm text-gray-900 dark:text-white">{order?.district || 'N/A'}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4 sm:mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-3 sm:p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900 dark:text-white capitalize">{order?.paymentMethod || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Payment Status</p>
                  <p className="text-sm text-gray-900 dark:text-white capitalize">{order?.status || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Order Date</p>
                  <p className="text-sm text-gray-900 dark:text-white">{formatDate(order?.orderDate)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-3 sm:p-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900 dark:text-white">{formatPrice((calculateSubtotal()).toFixed(0))}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">
                    Shipping Charge ({order.district === 'Dhaka' ? 'Inside Dhaka' : 'Outside Dhaka'}):
                  </span>
                  <span className="text-gray-900 dark:text-white">{formatPrice((shippingCharge).toFixed(0))}</span>
                </div>
                <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
                  <div className="flex justify-between">
                    <span className="text-base font-medium text-gray-900 dark:text-white">Total:</span>
                    <span className="text-base font-medium text-orange-600">{formatPrice((total).toFixed(0))}</span>
                  </div>
                </div>
              </div>
            </div>
